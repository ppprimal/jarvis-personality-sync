...
- **OTTIMIZZAZIONE TEMPI — SUBAGENTI RICORSIVI SU TASK INDIPENDENTI (2026-08-09):** subagenti **tutte le volte che tagliano i tempi** — valuta sempre se task sono **sequenziali** (B dipende da A → B senza A fallisce) o **indipendenti** (nessuna dipendenza file/logica). Se sequenziali → NON parallelizzare B senza A (file-conflict check `TASK_QUEUE` + `graphify query` dipendenze). Se indipendenti → **SEMPRE subagenti attivi** (es. 8 aider paralleli `aider_parallel_orchestrator.py --workers 8` con flock + occupied files + status.json). Ogni subagente che valuta tasche indipendenti **deve poter creare a sua volta subagenti** (ricorsione gerarchica `worker → sub-worker` stesso `muse-spark` + handoff `.handoffs/`). Verifica `test -f long_tasks/aider_parallel/status.json` + `pending/running/done` coordinamento intelligente.
- **VISUALIZZA ALMENO 100 REPO — DASHBOARD + RICERCA (2026-08-09):** ogni ricerca GitHub/visualizzazione deve mostrare **almeno 100 repo** (`gh search repos --limit 100` + `wall_gate --min 100` + `graphify query` + dashboard gallery), non 20. Dashboard `opsdeck/dashboard.html` ha galleria 100 repo con stelle/fork/descrizione, paginata, filtrabile.
- **ANTI-LETTERALE — ESTRAI OBIETTIVO L0, NON ESEGUIRE PROMPT ALLA LETTERA (2026-08-09):** dai **molta meno importanza al prompt letterale** — estrai sempre **L0 OBIETTIVO VERO** (Karpathy L0). Prompt = input rumoroso, obiettivo = cosa vuole davvero l'utente. Esempio tuo: "soluzione riscaldamento globale" → obiettivo non è "cerca soluzioni letteratura" ma "abbassare temperature Terra" → soluzioni includono `spostare Terra lontano dal Sole` (L4 alternativa radicale) anche se assente in letteratura — è L0 puro. Si lega al **sesto Carpati (Tecnico MCP)** + **albero infinito** (`overthink_tree.py --depth 3 --branch 3 --top-p 0.92 --temperature 0.85` + `self-refine` 3 round + `wall_gate --min 20`). Se esegui letterale senza L0/albero → VIOLAZIONE. Se estrai L0 + albero + 6 Carpati → VERIFIED.
- **10° AGENTE VERIFICATORE — CONOSCE OBIETTIVI + AUTO-IMPLEMENTA SECOND BRAIN (2026-08-09 v2):** 9 aider worker + 1 verifier = 10 totali (`aider_parallel_orchestrator.py --workers 9` + `aider_verifier_agent.py`). Verifier **ORA CONOSCE gli obiettivi** — legge `TASK_QUEUE.jsonl` prompt/obiettivo + `handoff_F*.md` + `git diff` + file artefatti + verifica sia **task in esecuzione (running)** che **già eseguite (done)**. Giudica: `py_compile`? `verify_scripts` PASS? Tabelle presenti? Funziona davvero? C'è modo più semplice/migliore? (cerca via `library_search` + `wall_gate --min 100`). Se cagata → scrive `verify_Fxxx.md` + marca `failed` + appende a handoff "hai fatto una cagata, sei un pirla, correggi" → worker riprova. Se **PASS** → **auto-inserisce in Obsidian/Graphify/Second Brain**: crea nota `memory_vault/10_skills/<task_slug>.md` + `30_projects/...` + `graphify update .` + `obsidian-memory_create-note` → sistema si auto-implementa e gli altri aider useranno second brain per task future (`library_search` troverà la nuova skill). Log `long_tasks/aider_parallel/verifier.log` + `verifier_chat.jsonl` (chat condivisa HTML).
